...
export default auth((req) => {
  ...
  // If Supabase is not configured (no status), allow access (development mode)
  if (isLoggedIn && !userStatus) {
    // Allow access to main app, but not the registration pending page
  ...
  // If logged in and authorized but on registration pending page, redirect to home
  if (isLoggedIn && userStatus === 'authorized' && isRegistrationPending) {
    return NextResponse.redirect(new URL("/", nextUrl));
  }
