...
const CGU_CGV_URL =
  "https://siomjdoyjuuwlpimzaju.supabase.co/storage/v1/object/public/front/legal/cgu_cgv_prophetic_orchestra_v2.md";

export default async function CguCgvPage() {
  let content = "";
  try {
    const res = await fetch(CGU_CGV_URL, { next: { revalidate: 3600 } });
    if (res.ok) {
        ...
        <article className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 sm:p-12">
          {content ? (
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Impossible de charger les CGU/CGV.
            </p>
          )}
        </article>
      </div>
