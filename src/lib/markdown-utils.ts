...
/** Escapes raw/decoded text before interpolating it into an HTML string. */
...
/**
 * Escapes only double-quotes for HTML attribute values whose content has already been
 * entity-encoded by the markdown parser (so we avoid double-encoding &lt; etc.).
 */
        ...
        if (codeContent.includes('▬') && codeContent.match(/#\d+\s+\w+/)) {
            ...
            if (rankingItems.length > 0) {
                ...
                rankingItems.forEach((item) => {
                    rankingHtml += `
            <div class="ranking-card group ${item.hasAnalysis ? 'cursor-pointer' : ''}" ${item.hasAnalysis ? `data-analysis data-analysis-query="${escapeAttr(item.name)}"` : ''}>
        ...
        if (hasExtendedRankings) {
            ...
            if (rankings.length > 0) {
                ...
                rankings.forEach((ranking) => {
                    rankingHtml += `
            <div class="extended-ranking-card group ${ranking.hasAnalysis ? 'cursor-pointer' : ''}" ${ranking.hasAnalysis ? `data-analysis data-analysis-query="${escapeAttr(ranking.name)}"` : ''}>
...
export function convertComparisonBarsToHtml(html: string): string {
    ...
    return html.replace(codeBlockRegex, (match, codeContent: string) => {
        ...
        <div class="comp-bar-row" style="--row-index: ${bar.index}">
          ...
          <span class="comp-bar-label">${escapeHtml(bar.label)}</span>
        </div>`;
        });
        output += '</div>';
    ...
    return html.replace(codeBlockRegex, (match, codeContent: string) => {
        ...
        points.forEach((pt: { label: string; xVal: number; yVal: number; isHighlighted: boolean }) => {
            ...
            <div class="scatter-plot-point${hlClass}" style="left: ${xPct.toFixed(1)}%; bottom: ${yPct.toFixed(1)}%">
              ...
              <span class="scatter-plot-point-label">${escapeHtml(pt.label)}</span>
            </div>`;
        });
        ...
        <div class="scatter-plot-container">
          ...
          ${xAxisTitle ? `<div class="scatter-plot-x-title">${escapeHtml(xAxisTitle)}</div>` : ''}
        </div>`;
    });
}
